...
1. **FIT Core：语言无界，算力随需**
   ...
   以「一切皆可替换」为核心理念，多语言函数计算底座（Java/Python/C++）支持插件化热插拔，独创智能聚散部署——代码无需修改，单体应用与分布式服务一键切换，
   运行时自动路由本地调用或 RPC，让基础设施成为「隐形的伙伴」。
...
- 一切皆可替换
  ...
  FIT 将稳定的服务定义与具体实现分离：`Genericable` 定义能力契约，`Fitable` 提供可替换实现。调用方无需感知实现采用何种语言、位于本地还是远程，也无需绑定
  具体插件。通过替换 `Fitable` 或插件组合，同一套业务可以适配不同技术栈、运行环境和部署形态，而无需修改调用代码。
...
您可以从 `docs` 目录查看项目的完整文档，文档包含框架的快速入门指南和用户指导手册。

- [FIT 快速入门指南](docs/framework/fit/quick-start-guide/01.%20构建基础%20Web%20应用.md)、[用户指导手册](docs/framework/fit/user-guide-book/01.%20插件%E3%80%81IoC%20容器和%20Bean.md)
- [Waterflow 快速入门指南](docs/framework/waterflow/quick-start-guide/01.%20介绍.md)、[用户指导手册](docs/framework/waterflow/user-guide-book.md)
- [FEL 快速入门指南](docs/framework/fel/quick-start-guide/01.%20模型.md)、[用户指导手册](docs/framework/fel/user-guide-book/01.%20AI%20流程.md)
