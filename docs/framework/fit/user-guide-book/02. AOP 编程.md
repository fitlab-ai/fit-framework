...
以下代码为例，匹配了在 `com.example.aop` 包及其子包下的所有类所有方法：
...
以下代码为例，匹配了代理对象是 org.fitframework.demo.MyService` 类的目标对象中的所有连接点：

```
this(org.fitframework.demo.MyService)
```
...
以下代码为例，匹配了是 `org.fitframework.demo.MyService` 类的目标对象的所有连接点：

```
target(org.fitframework.demo.MyService)
```
...
```
@annotation(org.fitframework.transaction.Transactional)
```
...
```
@args(com.example.security.Classified)
```
...
```
@target(org.fitframework.transaction.Transactional)
```
...
```
@within(org.fitframework.transaction.Transactional)
```
...
```
@params(com.example.MyAnnotation)
```
...
包：`org.fitframework.aop.interceptor.aspect.parser`
...
包：`org.fitframework.aop.interceptor`
...
4.`@AfterReturning`
5.`@AfterThrowing`
当不论是否在同一个切面类中的同类型增强（建议）方法想要在同一个连接点执行时，默认的执行顺序是未定义的。默认情况下，如果两个不同切面的同一类型的拦截器在同一个连接点执行时，会有两种都合理的执行情况（方法1前方法2后或者方法2前方法1后）。但在一些特殊场景下，我们要求两者之间有严格的执行顺序，此时我们可以通过设置优先级来控制执行顺序，即通过 `org.fitframework.annotation.Order` 注解实现。以下代码为例：
