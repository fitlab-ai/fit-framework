...
```
[yyyy-MM-dd hh:mm:ss.SSS] [INFO ] [main] [org.fitframework.runtime.direct.DirectFitRuntime] Prepare to start FIT application...
[yyyy-MM-dd hh:mm:ss.SSS] [INFO ] [main] [org.fitframework.runtime.direct.DirectFitRuntime] FIT application started.
...
这个示例体现了 FIT「一切皆可替换」的基本模型：`Weather` 是稳定的服务契约，`default-weather` 和 `other-weather` 是可替换实现，调用方始终面向同一个服务定义编程。
