...
- 注解：`@org.fitframework.retry.annotation.Retryable`、`@org.fitframework.retry.annotation.Recover`、`@org.fitframework.retry.annotation.Backoff`
- 代码入口：`org.fitframework.retry.RetryExecutor`
...
- 注解：`@org.fitframework.schedule.annotation.Scheduled`
- 代码入口：`org.fitframework.schedule.ThreadPoolScheduler`
- 使用详情：请参考类注释以及相关静态工厂方法。
- 备注：定时器支持`CRON`表达式。
...
- 注解：`@org.fitframework.transaction.Transactional`
- 代码入口：`org.fitframework.transaction.TransactionManager`
- 使用详情：请参考类注释以及相关静态工厂方法。
...
- 注解：`@org.fitframework.validation.constraints.NotBlank`、`@org.fitframework.validation.constraints.NotEmpty`、`@org.fitframework.validation.constraints.Range`等
- 代码入口：`org.fitframework.validation.ConstraintValidator`
- 使用详情：请参考类注释以及相关静态工厂方法。
...
- 备注：该工具类包含了非常多常用的工具方法，可以通过包路径及类名判定其功能范围，例如：`org.fitframework.util.StringUtils`就包含了所有字符串相关的工具方法。
