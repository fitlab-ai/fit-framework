...
在`log`插件内，我们通过`@Aspect`注解来定义一个日志管理的切面，同时打上`@Component`注解使之注册为一个`Bean`，通过`@Around("@annotation(org.fitframework.http.annotation.GetMapping)")`来拦截`GetMapping`注解，当执行到含该注解的方法时，该切面会自动拦截方法并执行相关逻辑。在本例中，切面拦截目标方法后实现了简单的日志功能。
...
```bash
[yyyy-MM-dd 00:00:00.000] [INFO ] [netty-request-assembler-thread-0] [org.fitframework.example.LoggingAspect] ===> org.fitframework.example.controller.MyController.hello() 开始执行
do something
[yyyy-MM-dd 00:00:00.000] [INFO ] [netty-request-assembler-thread-0] [org.fitframework.example.LoggingAspect] <=== org.fitframework.example.controller.MyController.hello() 执行成功 | 耗时: 1809ms
```