...
import org.fitframework.flowable.solo.AnySolo;
import org.fitframework.flowable.solo.PublisherSoloAdapter;
import org.fitframework.flowable.subscriber.EmptySubscriber;
import org.fitframework.flowable.util.OnSubscribedObserver;
 ...
 * @since 2024-02-11
 */
public interface Solo<T> extends Publisher<T>, java.util.concurrent.Flow.Publisher<T> {
    /**
    ...
    static <T> Solo<T> create(Consumer<Emitter<T>> consumer) {
        return fromPublisher(new FlexibleEmitterChoir<>(Emitter::create,
                consumer,
    ...
    static <T> Solo<T> fromEmitter(Emitter<T> emitter) {
        return fromPublisher(new FlexibleEmitterChoir<>(() -> emitter,
                null,
                OnSubscribedObserver::notifyOnSubscribed,
                null,
                null));
    ...
    /**
     * 直接使用 {@link EmptySubscriber} 订阅响应式流。
     */
    void subscribe();
