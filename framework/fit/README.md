...
FIT 通过稳定的服务契约隔离调用方与具体实现。一个 `Genericable` 可以对应多个 `Fitable`，不同实现可以来自不同插件、使用不同语言，并采用不同部署方式。替换实现时，
调用方及其业务逻辑无需随之修改。
...
``` java
package org.fitframework.demo1;
...
``` java
package org.fitframework.demo1.controller;
