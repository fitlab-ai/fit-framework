// SPDX-License-Identifier: MIT
// Copyright (c) 2024 Huawei Technologies Co., Ltd.
// Copyright (c) 2026 The FIT Lab AI Group

package org.fitframework.http.annotation;

import org.fitframework.http.server.handler.Source;
import org.fitframework.annotation.Forward;
import org.fitframework.util.StringUtils;

import java.lang.annotation.Documented;
import java.lang.annotation.ElementType;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
import java.lang.annotation.Target;

/**
 * 表示 REST 接口的请求映射中的表单参数。
 *
 * @author 邬涨财
 * @since 2023-11-20
 */
@Documented
@Retention(RetentionPolicy.RUNTIME)
@Target({ElementType.PARAMETER, ElementType.ANNOTATION_TYPE, ElementType.FIELD})
@RequestParam(in = Source.FORM)
public @interface RequestForm {
    /**
     * 获取表单参数的名字。
     *
     * @return 表示表单参数名字的 {@link String}。
     * @see #name()
     */
    @Forward(annotation = RequestForm.class, property = "name") String value() default StringUtils.EMPTY;

    /**
     * 获取表单参数的名字。
     *
     * @return 表示表单参数名字的 {@link String}。
     */
    @Forward(annotation = RequestParam.class, property = "name") String name() default StringUtils.EMPTY;

    /**
     * 获取表单参数是否为必须的标志。
     *
     * @return 如果表单参数必须存在，则返回 {@code true}，否则，返回 {@code false}。
     */
    @Forward(annotation = RequestParam.class, property = "required") boolean required() default true;

    /**
     * 获取表单参数的默认值。
     *
     * @return 表示表单参数的默认值的 {@link String}。
     */
    @Forward(annotation = RequestParam.class,
            property = "defaultValue") String defaultValue() default DefaultValue.VALUE;
}
