...
- **`fit start` 命令启动方式**：
  ...
  - 如果需要使用 Nacos 版注册中心，操作步骤：
    ...
    2. **放入 build/plugins 目录**：
       ```bash
       cp fit-builtin/plugins/fit-service-coordination-nacos/target/fit-service-coordination-nacos-3.7.0-SNAPSHOT.jar ../../../build/plugins/
       ```
    3. **移除 Simple 插件**：
       ```bash
       rm ../../../build/plugins/fit-service-coordination-simple-3.7.0-SNAPSHOT.jar
       ```
...
**服务接口定义**（`service/src/main/java/com.example/fit/example/Weather.java`）：
```java
...
**服务实现**（`app-default-weather/src/main/java/com.example/fit/example/DefaultWeather.java`）：
```java
@Component
...
**服务调用**（`app-assistant/src/main/java/com.example/fit/example/controller/AssistantController.java`）：
```java
@Component
