...
/**
 * {@link DefaultConsumerComposite} 的单元测试。
 *
 * @author 季聿阶
 * @since 2022-02-17
 */
@DisplayName("验证 DefaultConsumerComposite")
public class DefaultConsumerCompositeTest {
    private ConsumerComposite<AtomicInteger> consumer;
    private Consumer<AtomicInteger> existConsumer;

    @BeforeEach
    void setup() {
        this.consumer = ConsumerComposite.createDefault();
    ...
    class TestAdd {
        ...
        void given1MoreConsumerThenGet2Consumers() {
            ...
            DefaultConsumerCompositeTest.this.consumer.add(anotherConsumer);
            List<Consumer<AtomicInteger>> actual = DefaultConsumerCompositeTest.this.consumer.getConsumers();
            assertThat(actual).isNotNull().hasSize(2);
            AtomicInteger result = new AtomicInteger();
            DefaultConsumerCompositeTest.this.consumer.accept(result);
    ...
    class TestAddAll {
        ...
        void given1MoreConsumerThenGet2Consumers() {
            Consumer<AtomicInteger> anotherConsumer = AtomicInteger::incrementAndGet;
            DefaultConsumerCompositeTest.this.consumer.addAll(Collections.singletonList(anotherConsumer));
            List<Consumer<AtomicInteger>> actual = DefaultConsumerCompositeTest.this.consumer.getConsumers();
            assertThat(actual).isNotNull().hasSize(2);
            AtomicInteger result = new AtomicInteger();
            DefaultConsumerCompositeTest.this.consumer.accept(result);
    ...
    class TestRemove {
        ...
        void givenRemove1ExistConsumerThenGet0Consumers() {
            DefaultConsumerCompositeTest.this.consumer.remove(DefaultConsumerCompositeTest.this.existConsumer);
            List<Consumer<AtomicInteger>> actual = DefaultConsumerCompositeTest.this.consumer.getConsumers();
    ...
    class TestRemoveAll {
        ...
        @DisplayName("当移除1个消费者时，一共有0个消费者")
        void givenRemove1ExistConsumerThenGet0Consumers() {
            DefaultConsumerCompositeTest.this.consumer.removeAll(
                    Collections.singletonList(DefaultConsumerCompositeTest.this.existConsumer));
            List<Consumer<AtomicInteger>> actual = DefaultConsumerCompositeTest.this.consumer.getConsumers();
            assertThat(actual).isNotNull().hasSize(0);
            AtomicInteger result = new AtomicInteger();
            DefaultConsumerCompositeTest.this.consumer.accept(result);
