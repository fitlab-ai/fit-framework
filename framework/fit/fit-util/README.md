...
import org.fitframework.model.tree.TreeNode;

public class Demo {
    public static void main(String[] args) {
        Tree<Boolean> tree = Tree.create('.');
        tree.put("org.fitframework", true);
        tree.put("org.fitframework.demo", true);
        print(tree);
        // com.example=null
        //   fit=true
        //   fitframework=null
...
JDK预置的`URLStreamHandler`都定义在`sun.net.www.protocol`包下，以其作为参考，我们将使用`org.fitframework.protocol`包作为对URL协议的支持。

因此，我们将通过`org.fitframework.protocol.jar.Handler`类型，提供对`jar`协议的扩展。
