...
```bash
cd alpine
PUSH_IMAGE=true ./build.sh 3.6.0 com.example/
```

**3. 批量推送所有镜像**
```bash
PUSH_IMAGE=true ./build_all.sh build 3.6.0 com.example/
```
...
# 构建并推送所有镜像
PUSH_IMAGE=true PARALLEL=4 ./build_all.sh build 3.6.1 com.example/
...
# 从仓库拉取
docker pull com.example/fit-framework:3.6.1-alpine

# 验证版本和功能
docker run --rm com.example/fit-framework:3.6.1-alpine fit help
```
    ...
    steps:
      ...
      - name: Build and push
        run: |
          ...
          PUSH_IMAGE=true ./build.sh ${{ steps.version.outputs.version }} com.example/
```
...
```bash
wget -O /tmp/test.zip "https://github.com/fitlab-ai/fit-framework/releases/download/v3.6.0/3.6.0.zip"
...
- **GitHub Issues**: https://github.com/fitlab-ai/fit-framework/issues
